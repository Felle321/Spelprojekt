    ...
    public class EffectObject
    {
        ...
        public void Draw(Point offset, Random rand, SpriteBatch spriteBatch, Camera camera)
        {
            ...
            switch (type)
            {
                case Type.idle:
                    if (hp > 0)
                        SpriteHandler.Draw(sprite, rand, spriteBatch, camera, position + offset.ToVector2() - new Vector2(SpriteHandler.sprites[sprite].width / 2 * scale, SpriteHandler.sprites[sprite].height / 2 * scale), scale, angle, position, color, 0f, SpriteEffects.None, 0.50f);
