...
namespace GymnasieArbete
{
    public abstract class Particle
    {
        ...
        public virtual void Draw(Point offset, Random rand, SpriteBatch spriteBatch, Camera camera, float depth)
        {
        }
        ...
        public class Smoke : Particle
        {
            ...
            public Smoke(int ID, Random rand, Vector2 position, Vector2 movement, float angle, float scale, float thickness, int duration, Color color, bool rise) : base(ID)
            {
            ...
            public override void Update(int ID, Room room, Random rand)
            {
                ...
                if (timer < duration * .2f * thickness && timer != 0)
                {
                    opacity = (float)timer / duration * .2f * thickness;
                }
                if (timer < duration * .4f * thickness && timer != 0)
                {
                    scale += (float)timer / duration * .4f / 50;
                }
            ...
            public override void Draw(Point offset, Random rand, SpriteBatch spriteBatch, Camera camera, float depth)
            {
                if (delay)
                    delay = false;
                else
        ...
        public class Fire : Particle
        {
            ...
            public Fire(int ID, Room room, Random rand, Vector2 position, Vector2 movement, float scale, float thickness, int duration, Color color) : base(ID)
            {
                ...
                if (rand.Next(12) != 0)
                    room.particles.Add(new Smoke(room.particles.Count, rand, position, new Vector2(movement.X, movement.Y / 2), angle, scale, thickness / 2, initialDuration * 2, Color.Black, true));
            }
            ...
            public override void Update(int ID, Room room, Random rand)
            {
                ...
                if (timer < 10 * thickness && timer != 0)
                {
                    opacity = ((float)timer / initialDuration * thickness) * .8f;
                }
        ...
        public class Static : Particle
        {
            ...
            public override void DrawLight(Point offset, Random rand, SpriteBatch spriteBatch, Camera camera)
            {
                SpriteHandler.Draw("gradient", rand, spriteBatch, camera, position, scaleLight, angle, new Vector2(128, 128), color, (timer / (startDuration * 1.2f)) * opacityLight, SpriteEffects.None, 0.5f);
            }
        ...
        public class Dynamic : GameObject
        {
            ...
            public Dynamic(Rectangle rectangle, int level, Room newRoom, Vector2 movement, Vector2 deAcceleration, string spriteKey, bool pixel, Color color, float scale, int duration, bool impact, bool additive, bool useGravity) : base(rectangle, level, newRoom, spriteKey)
            {
        ...
        public class Spark : Particle.Bounce
        {
            int duration, timer, size;
            float scale;
            float Scale
            {
                get
                {
                    return scale + scaleOffsets[(timer / 2) % scaleOffsets.Length];
                }
            }

            bool usePixel;
            float opacity;
            float[] scaleOffsets = new float[20];

            public Spark(Random rand, Rectangle rectangle, int level, Room newRoom, Vector2 movement, bool useGravity, bool useResistance, bool pixel, Color color, int duration, float intensity) : base(rectangle, level, newRoom, movement, Vector2.Zero, .9f, useGravity, useResistance, "particle_spark", pixel, color, rectangle.Width, duration, false, false)
            {
                sprite = "particle_spark";
                spriteLight = "particle_spark_light";
                this.duration = duration;
                timer = duration;
                usePixel = pixel;
                ...
                {
                    this.scale = (float)size / 32;
                }
                opacity = intensity;

                for (int i = 0; i < scaleOffsets.Length; i++)
                {
                    scaleOffsets[i] = (rand.Next(20) - 10) * .04f;
                }
            }

            public Spark(Random rand, Rectangle rectangle, int level, Room newRoom, Vector2 movement, Vector2 deAcceleration, float bounceFactor, bool useGravity, bool useResistance, bool pixel, Color color, int duration, bool impact, bool additive, float intensity) : base(rectangle, level, newRoom, movement, deAcceleration, bounceFactor, useGravity, useResistance, "particle_spark", pixel, color, rectangle.Width, duration, impact, additive)
            {
                sprite = "particle_spark";
                spriteLight = "particle_spark_light";
                this.duration = duration;
            ...
            {
                if (usePixel)
                    spriteBatch.Draw(pixel, position - new Vector2(size / 2, size / 2) + offset.ToVector2(), null, color * opacity, 0f, new Vector2(size / 2, size / 2), Scale, SpriteEffects.None, 0f);
                else
                    SpriteHandler.Draw(sprite, rand, spriteBatch, camera, position - new Vector2(16 * Scale * .8f, 16 * Scale * .8f) + offset.ToVector2(), Scale * .8f, 0, Vector2.Zero, color, opacity, SpriteEffects.None, 0f);
            }

            public override void DrawLight(Point offset, Random rand, SpriteBatch spriteBatch, Camera camera)
            {
                SpriteHandler.Draw(spriteLight, rand, spriteBatch, camera, position - new Vector2(128 * Scale, 128 * Scale), Scale * 2, 0, Vector2.Zero, color, opacity * .6f, SpriteEffects.None, 0f);
            }
        }
    }
}