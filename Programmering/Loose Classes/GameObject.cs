...
namespace GymnasieArbete
{
    public abstract class GameObject
    {
        ...
        public GameObject(int ID, Rectangle rectangle, int level, Room newRoom, string spriteKey)
        {
            this.ID = ID;
        ...
        public virtual void Update(Room room, Random rand)
        {
            if (live)
            {
                ...
                textureOffset = room.MoveObject(ref position, Rectangle, ref movement, ref onGround, ref onWall, ref onWallRight, onGroundPrev, ref slope, ref onSlope, bounceFactor);
            }
            else
            {
                Die();
        ...
        {
            if (sprite != "")
                SpriteHandler.Draw(sprite, rand, spriteBatch, camera, position);
        }
        ...
        {
            if(spriteLight != null)
                SpriteHandler.Draw(spriteLight, rand, spriteBatch, camera, position);
        }
        ...
        public virtual void Damage(Room room, int sourceID, float damage)
        {
            hp -= (int)Math.Round(damage);
            if (World.displayDamageGameObjects)
                room.effectObjects.Add(new EffectObject(position, movement, World.fontDamage, ((int)Math.Round(damage)).ToString(), 40));
        }
        ...
        public virtual void UpdateDamageZones(Room room, int ID)
        {
            for (int i = 0; i < damageZones.Count; i++)
            {
                if (damageZones[i].hp <= 0)
                    damageZones.RemoveAt(i);
                damageZones[i].Update(room, ID);
            }
        }
    }
}