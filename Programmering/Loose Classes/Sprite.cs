    ...
    public class Sprite
    {
            ...
            if (!File.Exists(directory + ".png"))
            {
                ...
                while (textures.Count < files.Length - 1 || width == 0)
                {
                    ...
                    else if (Path.GetExtension(files[j]) == ".txt")
                    {
                        ...
                        speed = float.Parse(info[2]);
                        scale = float.Parse(info[3]);
                        if (info[4].Contains("true"))
                            random = true;
                        randFrequency = (int)(float.Parse(info[5]) * 60);
                        randDeviation = (int)(float.Parse(info[6]) * 60);
        ...
        public void Draw(Random rand, SpriteBatch spriteBatch, Camera camera, Vector2 position)
        {
            if (draw && (camera.rectangle.Intersects(new Rectangle(Game1.CeilAdv(position.X), Game1.CeilAdv(position.Y), width, height)) || HUD))
            {
                if (!animated)
                {
                    spriteBatch.Draw(textures[0], position, null, GetColorOpaque(color, opacity), angle, origin, scale, spriteEffect, 0f);
                }
                else
                {
                    if (spriteSheet[(int)Math.Floor(currentTexture)])
                    {
                        ...
                        spriteBatch.Draw(textures[(int)Math.Floor(currentTexture)], position, new Rectangle(x, y, width, height), GetColorOpaque(color, opacity), angle, origin, scale, spriteEffect, 0f);
                    }
                    else
                    {
                        spriteBatch.Draw(textures[(int)Math.Floor(currentTexture)], position, null, GetColorOpaque(color, opacity), angle, origin, scale, spriteEffect, 0f);
                    }
                }
            ...
            else
            {
                if (random)
                {
                    for (int i = 0; i < children.Count; i++)
                    {
                        ...
                        if (children[i].draw)
                        {
                            children[i].Draw(rand, spriteBatch, camera, position);
        ...
        public void Draw(Random rand, SpriteBatch spriteBatch, Camera camera, Vector2 position, Vector2 vector2Scale)
        {
            if (draw && (camera.rectangle.Intersects(new Rectangle(Game1.CeilAdv(position.X), Game1.CeilAdv(position.Y), width, height)) || HUD))
            {
                if (!animated)
                {
                    spriteBatch.Draw(textures[0], position, null, GetColorOpaque(color, opacity), angle, Vector2.Zero, scale, spriteEffect, 0f);
                }
                else
                {
                    if (spriteSheet[(int)Math.Floor(currentTexture)])
                    {
                        int x = (int)(currentFrame % Math.Floor((float)(textures[(int)Math.Floor(currentTexture)].Width / width))) * width;
                        int y = (int)Math.Floor(currentFrame / (textures[(int)Math.Floor(currentTexture)].Width / width)) * height;
                        spriteBatch.Draw(textures[(int)Math.Floor(currentTexture)], position, new Rectangle(x, y, width, height), GetColorOpaque(color, opacity), angle, origin, vector2Scale, spriteEffect, 0f);
                    }
                    else
                    {
                        spriteBatch.Draw(textures[(int)Math.Floor(currentTexture)], position, null, GetColorOpaque(color, opacity), angle, origin, vector2Scale, spriteEffect, 0f);
                    }
                }
                ...
                if (random)
                {
                    for (int i = 0; i < children.Count; i++)
                    {
                        if (children[i].draw)
                        {
                            ...
                            children[i].origin = origin;
                            children[i].color = color;
                            children[i].Draw(rand, spriteBatch, camera, position, vector2Scale);
                            if (children[i].AnimationEnd)
                            {
                                draw = true;
                                children[i].draw = false;
        ...
        public void Draw(Random rand, SpriteBatch spriteBatch, Camera camera, Vector2 position, float scale, float angle, Vector2 origin, Color color, float opacity)
        {
            if (draw && (camera.rectangle.Intersects(new Rectangle(Game1.CeilAdv(position.X), Game1.CeilAdv(position.Y), width, height)) || HUD))
            {
                if (!animated)
                {
                    spriteBatch.Draw(textures[0], position, null, GetColorOpaque(color, opacity), angle, origin, scale * this.scale, spriteEffect, 0f);
                }
                else
                {
                    if (spriteSheet[(int)Math.Floor(currentTexture)])
                    {
                        int x = (int)(currentFrame % Math.Floor((float)(textures[(int)Math.Floor(currentTexture)].Width / width))) * width;
                        int y = (int)Math.Floor(currentFrame / (textures[(int)Math.Floor(currentTexture)].Width / width)) * height;
                        spriteBatch.Draw(textures[(int)Math.Floor(currentTexture)], position, new Rectangle(x, y, width, height), GetColorOpaque(color, opacity), angle, origin, scale * this.scale, spriteEffect, 0f);
                    }
                    else
                    {
                        spriteBatch.Draw(textures[(int)Math.Floor(currentTexture)], position, null, GetColorOpaque(color, opacity), angle, origin, scale * this.scale, spriteEffect, 0f);
                    }
                }
        ...
        /// <param name="origin">The origin of the rotation</param>
        /// <param name="color">The color of the sprite</param>
        public void Draw(Random rand, SpriteBatch spriteBatch, Camera camera, Vector2 position, Vector2 scale, float angle, Vector2 origin, Color color, float opacity)
        {
            if (draw && (camera.rectangle.Intersects(new Rectangle(Game1.CeilAdv(position.X), Game1.CeilAdv(position.Y), width, height)) || HUD))
            {
                if (!animated)
            ...
            else
            {
                if (random)
                {
                    for (int i = 0; i < children.Count; i++)
                    {
                        ...
                        if (children[i].draw)
                        {
                            children[i].Draw(rand, spriteBatch, camera, position, this.scale * scale);
                            if (children[i].AnimationEnd)
                            {
                                draw = true;
                                children[i].draw = false;
        ...
        /// <param name="camera">The active camera</param>
        /// <param name="position">The position of the Sprite</param>
        public void Draw(Random rand, SpriteBatch spriteBatch, Camera camera, Vector2 position, Animation animation)
        {
            if (draw && (camera.rectangle.Intersects(new Rectangle(Game1.CeilAdv(position.X), Game1.CeilAdv(position.Y), width, height)) || HUD))
            {
                if (!animated)
                ...
                else
                {
                    if (spriteSheet[(int)Math.Floor(animation.currentTexture)])
                    {
                        ...
                        spriteBatch.Draw(textures[(int)Math.Floor(animation.currentTexture)], position, new Rectangle(x, y, width, height), GetColorOpaque(color, opacity), angle, origin, scale, spriteEffect, 0f);
                    }
                    else
                    {
                        spriteBatch.Draw(textures[(int)Math.Floor(animation.currentTexture)], position, null, GetColorOpaque(color, opacity), angle, origin, scale, spriteEffect, 0f);
                    }
                }
        ...
        /// <param name="position">The position of the Pprite</param>
        /// <param name="vector2Scale">The Vector2 value to rescale the sprite with</param>
        public void Draw(Random rand, SpriteBatch spriteBatch, Camera camera, Vector2 position, Vector2 vector2Scale, Animation animation)
        {
            if (draw && (camera.rectangle.Intersects(new Rectangle(Game1.CeilAdv(position.X), Game1.CeilAdv(position.Y), width, height)) || HUD))
            {
                if (!animated)
                ...
                else
                {
                    if (spriteSheet[(int)Math.Floor(animation.currentTexture)])
                    {
                        int x = (int)(animation.currentFrame % Math.Floor((float)(textures[(int)Math.Floor(animation.currentTexture)].Width / width))) * width;
                        int y = (int)Math.Floor(animation.currentFrame / (textures[(int)Math.Floor(animation.currentTexture)].Width / width)) * height;
                        spriteBatch.Draw(textures[(int)Math.Floor(animation.currentTexture)], position, new Rectangle(x, y, width, height), GetColorOpaque(color, opacity), angle, origin, vector2Scale, spriteEffect, 0f);
                    }
                    else
                    {
                        spriteBatch.Draw(textures[(int)Math.Floor(animation.currentTexture)], position, null, GetColorOpaque(color, opacity), angle, origin, vector2Scale, spriteEffect, 0f);
                    }
                }
        ...
        /// <param name="origin">The origin of the rotation</param>
        /// <param name="color">The color of the sprite</param>
        public void Draw(Random rand, SpriteBatch spriteBatch, Camera camera, Vector2 position, float scale, float angle, Vector2 origin, Color color, float opacity, Animation animation)
        {
            if (draw && (camera.rectangle.Intersects(new Rectangle(Game1.CeilAdv(position.X), Game1.CeilAdv(position.Y), width, height)) || HUD))
            {
                if (!animated)
                ...
                else
                {
                    if (spriteSheet[(int)Math.Floor(animation.currentTexture)])
                    {
                        int x = (int)(animation.currentFrame % Math.Floor((float)(textures[(int)Math.Floor(animation.currentTexture)].Width / width))) * width;
                        int y = (int)Math.Floor(animation.currentFrame / (textures[(int)Math.Floor(animation.currentTexture)].Width / width)) * height;
                        spriteBatch.Draw(textures[(int)Math.Floor(animation.currentTexture)], position, new Rectangle(x, y, width, height), GetColorOpaque(color, opacity), angle, origin, scale * this.scale, spriteEffect, 0f);
                    }
                    else
                    {
                        spriteBatch.Draw(textures[(int)Math.Floor(animation.currentTexture)], position, null, GetColorOpaque(color, opacity), angle, origin, scale * this.scale, spriteEffect, 0f);
                    }
                }
        ...
        /// <param name="origin">The origin of the rotation</param>
        /// <param name="color">The color of the sprite</param>
        public void Draw(Random rand, SpriteBatch spriteBatch, Camera camera, Vector2 position, Vector2 scale, float angle, Vector2 origin, Color color, float opacity, Animation animation)
        {
            if (draw && (camera.rectangle.Intersects(new Rectangle(Game1.CeilAdv(position.X), Game1.CeilAdv(position.Y), width, height)) || HUD))
            {
                if (!animated)
