    ...
    public abstract class Particle
    {
        ...
        public class Smoke : Particle
        {
            ...
            public override void Draw(Point offset, Random rand, SpriteBatch spriteBatch, Camera camera, float depth)
            {
                if (delay)
                    ...
                    SpriteHandler.Draw(sprite, rand, spriteBatch, camera, position + offset.ToVector2(), scale, angle, origin, color, opacity, SpriteEffects.None, depth);
            }
        }
        ...
        public class Fire : Particle
        {
            ...
            public override void Draw(Point offset, Random rand, SpriteBatch spriteBatch, Camera camera, float depth)
            {
                ...
                {
                    SpriteHandler.Draw(sprite, rand, spriteBatch, camera, position + offset.ToVector2(), scale, angle, origin, color, opacity, SpriteEffects.None, spriteNum);
                }
            }

            public override void DrawLight(Point offset, Random rand, SpriteBatch spriteBatch, Camera camera)
            {
                SpriteHandler.Draw("gradient", rand, spriteBatch, camera, position+ offset.ToVector2(), scale, 0, origin, color, opacityLight, SpriteEffects.None, 0);
            }
        }
        ...
        public class Static : Particle
        {
            ...
            {
                if(scale == 0)
                    SpriteHandler.Draw(spriteKey, rand, spriteBatch, camera, position + offset.ToVector2(), scaleVector2, angle, new Vector2(SpriteHandler.sprites[spriteKey].width / 2, SpriteHandler.sprites[spriteKey].height / 2), Color.White, 1f, SpriteEffects.None, depth);
                else
                    SpriteHandler.Draw(spriteKey, rand, spriteBatch, camera, position + offset.ToVector2(), scale, angle, new Vector2(SpriteHandler.sprites[spriteKey].width/2,SpriteHandler.sprites[spriteKey].height/2), Color.White, 1f, SpriteEffects.None, depth);
            }

            public override void DrawLight(Point offset, Random rand, SpriteBatch spriteBatch, Camera camera)
            {
                SpriteHandler.Draw("gradient", rand, spriteBatch, camera, position + offset.ToVector2(), scaleLight, angle, new Vector2(128, 128), color, (duration / (startDuration * 1.2f)) * opacityLight, SpriteEffects.None, 0.5f);
            }
        ...
        public class Dynamic : GameObject
        {
            ...
            public override void Draw(Point offset, Random rand, SpriteBatch spriteBatch, GraphicsDevice graphics, Camera camera)
            {
                if (sprite == null)
                {
                    spriteBatch.Draw(pixel, new Vector2(position.X + offset.X, position.Y + offset.Y), null, color, angle, Vector2.Zero, new Vector2(Rectangle.Width * scale, Rectangle.Height * scale), SpriteEffects.None, 0f);
                }
                else
