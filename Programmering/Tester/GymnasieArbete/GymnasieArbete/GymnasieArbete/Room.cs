    ...
    public class Room
    {
        ...
        public Room(int ID, Random rand, bool windBool, float windMagnitude, RoomReference roomReference, Room baseRoom)
        {
            ...
            if (roomReference != null && baseRoom != null)
            {
                ...
                doors = roomReference.doors;

                for (int i = 0; i < doors.Count; i++)
                {
                    doors[i].rectangle = new Rectangle(platforms[doors[i].wall].rectangle.X - 6, platforms[doors[i].wall].rectangle.Bottom - World.doorHeight, platforms[doors[i].wall].rectangle.Width + 10, World.doorHeight);
                }
            }
        }
        ...
        public void Draw(Point offset, Random rand, SpriteBatch spriteBatch, GraphicsDevice graphics, Camera camera)
        {
            ...
            for (int i = 0; i < gameObjects.Count; i++)
            {
                gameObjects[i].Draw(offset, rand, spriteBatch, graphics, camera);
            }
            for (int i = 0; i < scraps.Count; i++)
            {
                scraps[i].Draw(offset, rand, spriteBatch, graphics, camera);
            }
            for (int i = 0; i < particles.Count; i++)
            {
