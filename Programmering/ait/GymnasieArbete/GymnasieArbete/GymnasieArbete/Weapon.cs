...
namespace GymnasieArbete
{
    ...
    public abstract class Weapon
    {
        ...
        /// <returns></returns>
        public virtual int ShootBullet(Vector2 pos, Vector2 movement, float damage, int range, List<Platform> platforms, List<GameObject> gameObjects, Color color, Player player)
        {
            ...
            public override void Update(Player player, Room room, bool leftHand)
            {
                ...
                if (e_attacks == e_Attacks.a2)
                {
                    ...
                    if (AttackUpgradeList[1][2] == 1)
                    {
                        ...
                        if (firstTime)
                        {
                            dmgZone = new DamageZone(new Rectangle((int)(player.position.X + player.width), (int)(player.position.Y -50), 350, player.height + 100), 1, 10, 0, player.ID, true, true, attackable);
                    ...
                    else if (AttackUpgradeList[1][1] == 1)
                    {
                        ...
                        if (firstTime)
                        {
                            dmgZone = new DamageZone(new Rectangle((int)(player.position.X + player.width), (int)(player.position.Y- 50), 200, player.height + 100), 1, 10, 0, player.ID, true, true, attackable);
                            room.damageZones.Add(dmgZone);
                            firstTime = false;
                        }
                        if (player.direction == 1)
                    ...
                    else if (AttackUpgradeList[1][0] == 1)
                    {
                        ...
                        if (attackTimer >= (float)(60 / (float)(AttackSpeed * 0.5f)))
                        {
                            ...
                            if (player.direction == 1)
                            {
                                room.damageZones.Add(new DamageZone(new Rectangle((int)ShootOrigin.X, (int)ShootOrigin.Y - 50, 200, 100), 4, 10, 10, room.damageZones.Count, true, false, attackable));
                            }
                            else if (player.direction == -1)
                            {
                                room.damageZones.Add(new DamageZone(new Rectangle((int)ShootOrigin.X - 200, (int)ShootOrigin.Y - 50, 200, 100), 4, 10, 10, room.damageZones.Count, true, false, attackable));
                            }
                ...
                if (e_attacks == e_Attacks.a3)
                {
                    if (AttackUpgradeList[2][1] == 1)
                    {
                        ...
                        {
                            room.gameObjects.Add(new ExplodingBall(room.gameObjects.Count, new Animation(spr_plasmaUlt), ShootOrigin, new Vector2(rand.Next(45, 50) * player.direction, rand.Next(-9, -8)), 30, 2, 60, room, true, false, true, true, 0f, true, spriteEffect, attackable, true));
                            room.gameObjects.Add(new ExplodingBall(room.gameObjects.Count, new Animation(spr_plasmaUlt), ShootOrigin, new Vector2(rand.Next(45, 50) * player.direction, rand.Next(-9, -8)), 30, 2, 60, room, true, false, true, true, 0f, true, spriteEffect, attackable, true));
                            attackTimer = 0;
                        }
                    }
                ...
                public override void Update(Room room, Random rand)
                {
                    ...
                    else if (!live)
                    {
                        ...
                        Game1.camera.AddShake(new Vector2(3,4), 2);
                            room.damageZones.Add(new DamageZone(new Rectangle((int)position.X - 50, (int)position.Y - 50, 200, 200), 30, 120, 30, ID, true, false, attackable));
                    }
                }
            }
            public override void Update(Player player, Room room, bool leftHand)
            {
                ...
                if (e_attacks == e_Attacks.a1)
                {
                    ...
                    if (AttackUpgradeList[0][2] == 1)
                    {
                        ...
                        if (attackTimer >= (float)(60 / (float)(AttackSpeed * attackspdDeviation)))
                        {
                            ShootBullet(ShootOrigin, new Vector2(player.direction, 0), Damage, 1000, room.platforms, room.gameObjects, Color.Yellow, player);
                            attackTimer = 0;
                        }
                    }
                    else if (AttackUpgradeList[0][1] == 1)
                ...
                if (e_attacks == e_Attacks.a2)
                {
                    ...
                    else if (AttackUpgradeList[1][1] == 1)
                    {
                        ...
                        if (attackTimer >= (float)(60 / (float)(AttackSpeed * attackspdDeviation)))
                        {
                            ...
                            SpriteHandler.sprites[spr_rShoot2_1].currentFrame = 0;
                            attackTimer = 0;
                            room.gameObjects.Add(new Shuriken(room.gameObjects.Count, new Animation("BAMF_Attack2_1"), ShootOrigin, new Vector2(13 * player.direction, 0), 5, 0, 60 * 2, room, true, false, false, false, 1f, true, true, spriteEffect, attackable, false));
                        }
                    }
                    else if (AttackUpgradeList[1][0] == 1)
                    {
                        ...
                        if (attackTimer >= (float)(60 / (float)(AttackSpeed * attackspdDeviation)))
                        {
                            ...
                            SpriteHandler.sprites[spr_rShoot2_1].currentFrame = 0;
                            attackTimer = 0;
                            room.gameObjects.Add(new Shuriken(room.gameObjects.Count, new Animation("BAMF_Attack2_1"), ShootOrigin, new Vector2(13 * player.direction, 0), 5, 0, 60 * 2, room, true, false, false, false, 1f, true, false, spriteEffect, attackable, false));
                        }
                    }
                }
                if (e_attacks == e_Attacks.a3)
                {
                    ...
                    if (AttackUpgradeList[2][1] == 1)
                    {
                        ...
                        {
                            attackTimer = 0;
                            room.gameObjects.Add(new Rocket(room.gameObjects.Count, new Animation("BAMF_Attack3_1"), ShootOrigin, new Vector2(10 * player.direction, 0), 4, 3, 60 * 3, room, true, false, false, false, 1f, true, false, 1, spriteEffect, attackable, true));
                            room.gameObjects.Add(new Rocket(room.gameObjects.Count, new Animation("BAMF_Attack3_1"), ShootOrigin, new Vector2(10 * player.direction, 0), 4, 3, 60 * 3, room, true, false, false, false, 1f, true, false, 2, spriteEffect, attackable, true));
                        }
                    }
                    else if (AttackUpgradeList[2][0] == 1)
                    {
                        ...
                        {
                            attackTimer = 0;
                            room.gameObjects.Add(new Rocket(room.gameObjects.Count, new Animation("BAMF_Attack3_1"), ShootOrigin, new Vector2(10 * player.direction, 0), 4, 3, 60 * 3, room, true, false, false, false, 1f, true, true, 1, spriteEffect, attackable, true));
                        }
                    }
                }
                keyboardPrev = Keyboard.GetState();
