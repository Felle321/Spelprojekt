...
namespace GymnasieArbete
{
    public abstract class Particle
    {
        ...
        public class Static : Particle
        {
            ...
            string spriteKey;
            float startDuration;
            public Static(int ID, string spriteKey, Random rand, Vector2 position, Vector2 movement, Vector2 deAcceleration, float scale, int duration, Color color) : base(ID)
            {
            ...
            public Static(int ID, string spriteKey, Random rand, Vector2 position, Vector2 movement, Vector2 deAcceleration, Vector2 scale, int duration, Color color) : base(ID)
            {
                this.spriteKey = spriteKey;
                type = Type.Smoke;
                this.position = position;
            ...
            public override void DrawLight(Random rand, SpriteBatch spriteBatch, Camera camera)
            {
                SpriteHandler.Draw("gradient", rand, spriteBatch, camera, position, 0.2f, angle, new Vector2(128, 128), color, (duration / (startDuration * 1.2f)) * opacityLight, SpriteEffects.None, 0.5f);
            }
