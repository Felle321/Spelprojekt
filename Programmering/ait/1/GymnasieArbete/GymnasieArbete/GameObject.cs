...
namespace GymnasieArbete
{
    public abstract class GameObject
    {
        ...
        public virtual void Update(Room room, Random rand)
        {
            if (live)
            {
                ...
                if (Math.Abs(movement.X) < .002f && movement.X != 0)
                    movement.X = 0;
                if ((Math.Abs(movement.Y) < .002f || onGround) && movement.Y > 0)
                    movement.Y = 0;

                if (onGround && useFriction)
                    movement.X *= World.friction * friction;
                ...
                if (useResistance)
                {
                    movement.X *= World.airResitance * airResitance;
                    movement.Y *= World.airResitance * airResitance;
                }
        ...
        public virtual void Draw(Random rand, SpriteBatch spriteBatch, Camera camera, GraphicsDevice graphicsDevice)
        {
            if (sprite != "")
