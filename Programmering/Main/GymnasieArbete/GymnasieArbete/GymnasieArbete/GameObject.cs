...
namespace GymnasieArbete
{
    public abstract class GameObject
    {
        ...
        public class Scrap : GameObject
        {
            ...
            public float floatTarget = 6;
            public float floatSpeed = 2f;

            public Scrap(Rectangle rectangle, int level, Room newRoom, string spriteKey, Vector2 movement, int value, int duration) : base(rectangle, level, newRoom, spriteKey)
            {
            ...
            public override void Update(Room room, Random rand)
            {
                ...
                if(spew && !used)
                {
                    ...
                    if (spewCounter % 5 == 0)
                    {
                        ...
                        if (total > 100)
                        {
                            room.scraps.Add(new Scrap(new Rectangle((int)position.X + Rectangle.Width / 2, (int)position.Y, SpriteHandler.sprites["Gear3"].width, SpriteHandler.sprites["Gear3"].height), 1, room, "Gear3", new Vector2(rand.Next(-10, 10), rand.Next(-7, -1)), 100, 300));
                            total -= 100;
                        }
                        if (total > 10 && total < 100)
                        {
                            room.scraps.Add(new Scrap(new Rectangle((int)position.X + Rectangle.Width / 2, (int)position.Y, SpriteHandler.sprites["Gear2"].width, SpriteHandler.sprites["Gear2"].height), 1, room, "Gear2", new Vector2(rand.Next(-10, 10), rand.Next(-7, -1)), 10, 300));
                            total -= 10;
                        }
                        if (total > 0 && total < 10)
                        {
                            room.scraps.Add(new Scrap(new Rectangle((int)position.X + Rectangle.Width / 2, (int)position.Y, SpriteHandler.sprites["Gear1"].width, SpriteHandler.sprites["Gear1"].height), 1, room, "Gear1", new Vector2(rand.Next(-10, 10), rand.Next(-7, -1)), 1, 300));
                            total -= 1;
                        }
                    }
                }
