...
namespace GymnasieArbete
{
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            player = new Player(new Rectangle(200, 500, 50, 110), 1, rooms[roomString]);
            ...
            rooms[roomString].gameObjects.Add(player);
            //MAIN FONTS
        ...
        protected override void Update(GameTime gameTime)
        {
            if (delay == 0)
            {
                ...
                if (!paused)
                {
                    ...
                    {
                        rooms[roomString].gameObjects.Add(new Enemy.GeminiBot(rand, new Vector2(100, 100), rooms[roomString], enemyLvl, new Animation("GeminiBot_Idle"), 2, 30));
                    }
                    if (keyboard.IsKeyDown(Keys.M) && keyboardPrev.IsKeyUp(Keys.M))
                    {
                        rooms[roomString].gameObjects.Add(new Enemy.LavaBot(rand, new Vector2(100, 100), rooms[roomString], enemyLvl, new Animation("LavaBot_Spray"), 2, 30));
                    }
                    if (keyboard.IsKeyDown(Keys.N) && keyboardPrev.IsKeyUp(Keys.N))
                    {
                        rooms[roomString].gameObjects.Add(new Enemy.GroundTroop(rand, new Vector2(100, 100), rooms[roomString], enemyLvl, new Animation("GroundTroop_Idle"), 2, 30));
                    }
                    if (keyboard.IsKeyDown(Keys.B) && keyboardPrev.IsKeyUp(Keys.B))
                    {
                        rooms[roomString].gameObjects.Add(new Enemy.BlizzardBot(rand, new Vector2(100, 100), rooms[roomString], enemyLvl, new Animation("BlizzardBot_Spray"), 2, 30));
                    }
                    ...
                    if (keyboard.IsKeyDown(Keys.T))
                    {
                        for (int i = 0; i < 3; i++)
                        {
                            rooms[roomString].particles.Add(new Particle.Fire(rooms[roomString].particles.Count, rooms[roomString], rand, player.position + new Vector2(10 + rand.Next(44), 20 + rand.Next(108)), Vector2.Zero, .5f, .8f, 40, new Color(230, 90 + rand.Next(40), 40)));
                        }
                    }
                ...
                ControllerPrev = GamePad.GetState(PlayerIndex.One);
        ...
        public void DrawGame(GameTime gameTime)
        {
            ...
            rooms[roomString].Draw(rand, spriteBatch, GraphicsDevice, camera);
            //rooms[roomString].DrawCollisions(GraphicsDevice, spriteBatch);
            //DrawDoors(GraphicsDevice, spriteBatch);
            
            player.Draw(rand, spriteBatch, GraphicsDevice, camera);
        ...
        public void DrawHUD(GameTime gameTime, Player player)
        {
            ...
            #region Debug
            spriteBatch.DrawString(fontDebug, player.hp.ToString(), new Vector2(100, 100), Color.Red);
            spriteBatch.DrawString(fontDebug, player.Scraps.ToString(), new Vector2(100, 50), Color.Red);
        ...
        public void LoadLevel(ContentManager Content)
        {
            ...
            for (int i = 0; i < roomFolders.Length; i++)
            {
                ...
                for (int j = 0; j < backgroundFiles.Length; j++)
                {
                    ...
                    rooms[roomString].backgrounds.Add(new Point(offsetX, offsetY), Texture2D.FromStream(GraphicsDevice, File.OpenRead(backgroundFiles[j])));
                }

                //Get lightSources
                //C: \Users\felix.dunmar\Dropbox\Skit\Rau\Library\levels\0\rooms\1\lightsources
                string[] lightSourcesFiles = Directory.GetFiles(LocationRoom + @"lightsources\");
                for (int j = 0; j < lightSourcesFiles.Length; j++)
                {
                    ...
                    rooms[roomString].lightSources.Add(new Point(offsetX, offsetY), Texture2D.FromStream(GraphicsDevice, File.OpenRead(lightSourcesFiles[j])));
                }
