...
namespace GymnasieArbete
{
    ...
    public abstract class Weapon
    {
        ...
        /// <returns></returns>
        public virtual int ShootBullet(Vector2 pos, Vector2 movement, float damage, int range, List<Rectangle> rectangles, List<GameObject> gameObjects, Color color, Player player)
        {
            ...
            for (int i = 0; i < range / 10; i++)
            {
                ...
                {
                    if(rectangles[j].Contains(pos.ToPoint()))
                    {
                        //Hit wall/floor
                ...
                if (e_attacks == e_Attacks.a1)
                {
                    ...
                    else if (AttackUpgradeList[0][0] == 1)
                    {
                        ...
                        if (attackTimer >= (float)(60 / (float)(AttackSpeed * 1.2)))
                        {
                            ...
                            ani.speed = (6 / temp) / 60;
                            room.gameObjects.Add(new Projectile(room.gameObjects.Count, ani, ShootOrigin - new Vector2(0, SpriteHandler.sprites[spr_energyballSmall].height / 4), new Vector2(15 * player.direction, 0), Damage, 0, temp, room, true, false, false, false, 0, true, spriteEffect, attackable));
                            attackTimer = 0;
                            Damage *= 10;
                        }
                    }
        ...
        {
            private Random rand;
            private string spr_plasmaBall, spr_smallParticle, spr_smallerParticle, spr_plasmaUlt;
            bool firstTime = true;
            ...
            {
                public List<GameObject.Types> attackable = new List<GameObject.Types>();
                public ExplodingBall(int ID, Animation ani, Vector2 pos, Vector2 vel, float damage, float knockback, float lifespan, Room room, bool friendly, bool useFriction, bool useGravity, bool useResistance, float bounceFactor, bool dieOnCollision, SpriteEffects spriteEffect, List<GameObject.Types> attackable) : base(ID, ani, pos, vel, damage, knockback, lifespan, room, friendly, useFriction, useGravity, useResistance, 0f, true, spriteEffect, attackable)
                {
                    this.attackable = attackable;
                }
                    ...
                    if (!live)
                    {
                        for (int i = 0; i < 1000; i++)
                        {
                            ...
                            if (random >= 6)
                            {
                                ...
                                plasmaSpew = new Particle.Static(room.gameObjects.Count, "Red_SmallParticle", rand, position, dirTemp * speedTemp, new Vector2(0.9f, 0.9f), 1f, (int)(rand.NextDouble() * rand.Next(5, 20)), Color.White);
                            }
                            else
                            {
                                plasmaSpew = new Particle.Static(room.gameObjects.Count, "Red_SmallerParticle", rand, position, new Vector2(rand.Next(-1, 2), rand.Next(-1, 2)) * new Vector2(rand.Next(1, 40), rand.Next(1, 40)), new Vector2(0.93f, 0.93f), 1f, (int)(rand.NextDouble() * rand.Next(5, 20)), Color.White);
                            }

                            room.particles.Add(plasmaSpew);
                        }
                        Game1.camera.AddShake(new Vector2(6, 8), 3);
                        room.damageZones.Add(new DamageZone(new Rectangle((int)position.X - 112, (int)position.Y - 112, 225, 225), 20, 60, 30, ID, true, false, attackable));
                        remove = true;
                    }
                }
            ...
            public override void Update(Player player, Room room, bool leftHand)
            {
                ...
                if (e_attacks == e_Attacks.a1)
                {
                    ...
                    else if (AttackUpgradeList[0][0] == 1)
                    {
                        ...
                        if (attackTimer >= (float)(60 / (float)(AttackSpeed * 0.5f)))
                        {
                            ...
                            room.gameObjects.Add(new Projectile(room.gameObjects.Count, ani, ShootOrigin - new Vector2(0, SpriteHandler.sprites[spr_plasmaBall].height / 3), new Vector2(rand.Next(17, 22) * player.direction, rand.Next(-12, -10)), Damage, 1, 2, room, true, false, true, false, 0, true, spriteEffect, attackable, 0f, 0.8f, 1f));
                            attackTimer = 0;
                        }
                    }
                    if (SpriteHandler.sprites[spr_rShoot1_1].animationEnd) { SpriteHandler.sprites[spr_rShoot1_1].speed = 0f; }
                ...
                if (e_attacks == e_Attacks.a2)
                {
                    ...
                    else if (AttackUpgradeList[1][0] == 1)
                    {
                        if (firstTime)
                        {
                            dmgZone = new DamageZone(new Rectangle((int)(player.position.X + player.width), (int)(player.position.Y), 200, player.height), 1, 10, 0, player.ID, true, true, attackable);
                        ...
                        for (int i = 0; i < 20; i++)
                        {
                            ...
                            if (random >= 6)
                            {
                                plasmaSpew = new Particle.Static(room.gameObjects.Count, spr_smallParticle, rand, ShootOrigin, new Vector2((rand.Next(3, 12) + Math.Abs(player.movement.X)) * player.direction, rand.Next(-8, 8) - player.movement.Y), new Vector2(0.93f, 0.93f), 1f, (int)(rand.NextDouble() * 30), Color.White);
                            }
                            else
                            {
                                plasmaSpew = new Particle.Static(room.gameObjects.Count, spr_smallerParticle, rand, ShootOrigin, new Vector2((rand.Next(3, 20) + Math.Abs(player.movement.X)) * player.direction, rand.Next(-10, 10) - player.movement.Y), new Vector2(0.93f, 0.93f), 1f, (int)(rand.NextDouble() * 60), Color.White);
                            }
                ...
                if (e_attacks == e_Attacks.a3)
                {
                    ...
                    else if (AttackUpgradeList[2][0] == 1)
                    {
                        ...
                        {
                            room.gameObjects.Add(new ExplodingBall(room.gameObjects.Count, new Animation(spr_plasmaUlt), ShootOrigin, new Vector2(rand.Next(45, 50) * player.direction, rand.Next(-9, -8)), 30, 2, 60, room, true, false, true, true, 0f, true, spriteEffect, attackable));
                            attackTimer = 0;
                        }
                    }
                }
        ...
        public class BAMF : Weapon
        {
            ...
            public BAMF(GraphicsDevice graphicsDevice) : base()
            {
                ...
                spr_lShoot3_1 = "BAMF_lShoot3_1";
                spr_rShoot1_1 = "BAMF_rShoot1_1";
                spr_rShoot2_1 = "BAMF_rShoot2_1";
                spr_rShoot3_1 = "BAMF_rShoot3_1";
                    ...
                    public override void Update(Room room, Player player)
                    {
                        if (hp > 0)
                        {
                            ...
                            if (attackable.Contains(GameObject.Types.Player))
                            {
                                ...
                                {
                                    player.TakeDamage(room, sourceID, damage);
                                    IDs.Add(player.ID);
            ...
            public class Rocket : Projectile
            {
                ...
                public Rocket(int ID, Animation ani, Vector2 pos, Vector2 vel, float damage, float knockback, float lifespan, Room room, bool friendly, bool useFriction, bool useGravity, bool useResistance, float bounceFactor, bool dieOnCollision, bool unstable, int rocketNum, SpriteEffects spriteEffect, List<GameObject.Types> attackable) : base(ID, ani, pos, vel, damage, knockback, lifespan, room, friendly, useFriction, useGravity, useResistance, 0f, true, spriteEffect, attackable)
                {
                    this.attackable = attackable;
                ...
                public override void Update(Room room, Random rand)
                {
                    ...
                    if (live)
                    {
                        if (unstable)
                        {
                            movement.Y += ((float)rand.NextDouble() * rand.Next(-5, 6));
                            if(movement.Y < 0) { movement.Y += 0.2f; }
                            else if(movement.Y > 0) { movement.Y -= 0.2f; }
                        }
                        else
                        {
                            //1 = the upper one
                    ...
                    else if (!live)
                    {
                        ...
                        for (int i = 0; i < 50; i++)
                        {
                            room.particles.Add(new Particle.Static(room.particles.Count, "BAMF_Particle", rand, position, new Vector2(rand.Next(-25, 25), rand.Next(-25, 25)), new Vector2(0.93f, 0.93f), new Vector2(rand.Next(1, 5), 1), rand.Next(1, 10), new Color(rand.Next(200, 255), rand.Next(115, 150), 55)));
                        }
                        Game1.camera.AddShake(new Vector2(3,4), 2);
                            room.damageZones.Add(new DamageZone(new Rectangle((int)position.X - 200, (int)position.Y - 200, 400, 400), 30, 120, 30, ID, true, false, attackable));
                    }
                }
            }
            public override void Update(Player player, Room room, bool leftHand)
            {
                ...
                if (e_attacks == e_Attacks.a1)
                {
                    ...
                    if (AttackUpgradeList[0][2] == 1)
                    {
                        int attackspdDeviation = 3;
                        if (attackTimer >= (float)(60 / (float)(AttackSpeed * attackspdDeviation)))
                        {
                            room.gameObjects.Add(new Projectile(room.gameObjects.Count, new Animation("BAMF_Attack1_1"), ShootOrigin, new Vector2(14 * player.direction, 0), 2, 1, 60 * 5, room, true, false, false, false, 0f, true, spriteEffect, attackable));
                        }
                    }
                    else if (AttackUpgradeList[0][1] == 1)
                    {
                        ...
                        if (attackTimer >= (float)(60 / (float)(AttackSpeed * attackspdDeviation)))
                        {
                            ...
                            {
                                attack1Counter++;
                                if (attack1Counter % 10 == 0)
                                {
                                    room.gameObjects.Add(new Projectile(room.gameObjects.Count, new Animation("BAMF_Attack1_1"), ShootOrigin, new Vector2(12 * player.direction, 0), 2, 1, 60 * 5, room, true, false, false, false, 0f, true, spriteEffect, attackable));
                                }
                                if (attack1Counter == 30)
                                {
                    ...
                    else if (AttackUpgradeList[0][0] == 1)
                    {
                        ...
                        {
                            attackTimer = 0;
                            ShootBullet(ShootOrigin, new Vector2(player.direction, 0), Damage, 1000, room.rectangles, room.gameObjects, Color.Yellow, player);
                        }
                    }
                }
                if (e_attacks == e_Attacks.a2)
                {
                    ...
                    else if (AttackUpgradeList[1][1] == 1)
                    {
                        ...
                        if (attackTimer >= (float)(60 / (float)(AttackSpeed * attackspdDeviation)))
                        {
                            ...
                            room.gameObjects.Add(new Shuriken(room.gameObjects.Count, new Animation("BAMF_Attack2_1"), ShootOrigin, new Vector2(13 * player.direction, 0), 5, 0, 60 * 2, room, true, false, false, false, 1f, true, spriteEffect, attackable));
                        }
                    }
                    else if (AttackUpgradeList[1][0] == 1)
                    {
                        ...
                        if (attackTimer >= (float)(60 / (float)(AttackSpeed * attackspdDeviation)))
                        {
                            ...
                            room.gameObjects.Add(new Projectile(room.gameObjects.Count, new Animation("BAMF_Attack2_1"), ShootOrigin, new Vector2(13 * player.direction, 0), 5, 0, 60 * 2, room, true, false, false, false, 1f, true, spriteEffect, attackable));
                        }
                    }
                }
                if (e_attacks == e_Attacks.a3)
                {
                    ...
                    if (AttackUpgradeList[2][1] == 1)
                    {
                        ...
                        {
                            attackTimer = 0;
                            room.gameObjects.Add(new Rocket(room.gameObjects.Count, new Animation("BAMF_Attack3_1"), ShootOrigin, new Vector2(10 * player.direction, 0), 4, 3, 60 * 3, room, true, false, false, false, 1f, true, false, 1, spriteEffect, attackable));
                            room.gameObjects.Add(new Rocket(room.gameObjects.Count, new Animation("BAMF_Attack3_1"), ShootOrigin, new Vector2(10 * player.direction, 0), 4, 3, 60 * 3, room, true, false, false, false, 1f, true, false, 2, spriteEffect, attackable));
                        }
                    }
                    else if (AttackUpgradeList[2][0] == 1)
                    {
                        ...
                        {
                            attackTimer = 0;
                            room.gameObjects.Add(new Rocket(room.gameObjects.Count, new Animation("BAMF_Attack3_1"), ShootOrigin, new Vector2(10 * player.direction, 0), 4, 3, 60 * 3, room, true, false, false, false, 1f, true, true, 1, spriteEffect, attackable));
                        }
                    }
                }
                keyboardPrev = Keyboard.GetState();
            ...
            public override void Update(Player player, Room room, bool leftHand)
            {
                ...
                if (e_attacks == e_Attacks.a2)
                {
                    ...
                    else if (AttackUpgradeList[1][1] == 1)
                    {
                        if (firstTime)
                        {
                            dmgZone = new DamageZone(new Rectangle((int)(player.position.X + player.width), (int)(player.position.Y), 300, player.height), 1, 10, 0, player.ID, true, true, attackable);
                            room.damageZones.Add(dmgZone);
                            firstTime = false;
                        }
                        if (player.direction == 1)
