...
namespace GymnasieArbete
{
    public class Platform
    {
            ...
            if (usable)
            {
                for (int i = 0; i < platforms.Count; i++)
                {
                    if (Game1.GetDistance(platforms[i].rectangle.ToPoint(), rectangle.ToPoint()) < 2000 && platforms[i].usable && platforms[i].rectangle.Y >= rectangle.Y && i != ID)
                    {
                        ...
                        {
                            if (platforms[i].rectangle.Right > rectangle.X && platforms[i].rectangle.X < rectangle.Right)
                            {
                                for (int j = 0; j < 10; j++)
                                {
                                    bool lPossible = true;
        ...
        public virtual void Draw(GraphicsDevice graphicsDevice, SpriteBatch spriteBatch)
        {
            ...
            {
                if (usable)
                    Game1.DrawRectangle(graphicsDevice, spriteBatch, rectangle, Color.Green);
                else
                    Game1.DrawRectangle(graphicsDevice, spriteBatch, rectangle, Color.Red);
            }
            else
            {
                if (usable)
                    Game1.DrawRectangle(graphicsDevice, spriteBatch, rectangle, Color.Blue);
                else
                    Game1.DrawRectangle(graphicsDevice, spriteBatch, rectangle, Color.Black);
            }
        }

        public virtual void DrawID(SpriteBatch spriteBatch)
        {
            spriteBatch.DrawString(Game1.fontDebug, ID.ToString(), rectangle.ToVector2(), Color.Blue);
        }
    }
}
