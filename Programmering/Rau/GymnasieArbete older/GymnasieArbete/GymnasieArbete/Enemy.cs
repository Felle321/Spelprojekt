...
namespace GymnasieArbete
{
    public abstract class Enemy : GameObject
    {
        ...
        public override void Draw(Random rand, SpriteBatch spriteBatch, GraphicsDevice graphicsDevice, Camera camera)
        {
        ...
        public virtual void GetPath(Player player, Room room)
        {
            ...
            path = room.GetPathInstructions(platformID, player.platformIDPrev, new Point(Rectangle.Width, Rectangle.Height), movementMax, maxPath);
            if (path.Count < 1)
            {
                stuck = true;
                Idle();
        ...
        public class GroundTroop : Enemy
        {
            ...
            /// <param name="room"></param>
            /// <returns></returns>
            public bool PlayerInSight1(Player player, Room room)
            {
                for (int i = 0; i < room.platforms.Count; i++)
                {
                    if (room.platforms[i].rectangle.Intersects(Game1.RectangleCreate(player.Rectangle.Center, EyePos.ToPoint())))
                ...
                if (Game1.LineIntersectsRect(EyePos, new Vector2(player.Rectangle.Center.ToVector2().X, EyePos.Y), player.Rectangle))
                {
                    return true;
                }
                else
            ...
            public override void Update(Room room, Random rand, Player player)
            {
                ...
                if (!jumpPause)
                {
                    ...
                    if (!stunned)
                    {
                        if (canAttack && state != State.charging && state != State.shooting && this.PlayerInSight1(player, room))
                        {
                            if (player.position.X > position.X)
                            {
                                direction = 1;
                        ...
                        DamageZone_Hitbox.Position = position;
                        if (canAttack && !charge && rand.Next(0, 1500) <= 10 && this.PlayerInSight1(player, room))
                        {
                            charge = true;
                            shootingTimer = 0;
                            UsePath = false;
                        }
                        else if (canAttack && !charge && shootingTimer >= 60 && this.PlayerInSight1(player , room))
                        {
                            shootingTimer = 0;
        ...
        public class LavaBot : Enemy
        {
            ...
            public class Lava : Particle.Dynamic
            {
                ...
                public override void Update(Room room, Random rand)
                {
                    ...
                    if (first)
                    {
                        ...
                        base.color = new Color(255, randomColor, 0);
                        first = false;

                    }
                    if (rand.Next(0, 300) <= 5)
                    {
                        switchDir = true;
                        //if(rand.Next(3) == 0)
                            //room.particles.Add(new Particle.Dynamic(Rectangle, 0, room, new Vector2(-4, rand.Next(4) - 2), ))
                    }
                    if (switchDir)
                    {
                        dir *= -1;
                    ...
                    if (Rectangle.Intersects(room.gameObjects[0].Rectangle) && dmgCounter >= 60)
                    {
                        room.gameObjects[0].hp -= (Damage / 200);
                        dmgCounter = 0;
                    }
                }
            ...
            public override void Update(Room room, Random rand, Player player)
            {
                ...
                else
                {
                    ...
                    jumpTimer++;
                    if (jumping.animationEnd) { jumping.speed = 0; }
                    if(jumpTimer >= 30) { base.Update(room, rand, player); }
                }
