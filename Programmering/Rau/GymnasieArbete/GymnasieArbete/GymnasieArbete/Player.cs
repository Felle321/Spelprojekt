...
namespace GymnasieArbete
{
    public class Player : GameObject
    {
        ...
        public override void Update(Room room, Random rand)
        {
                ...
                for (int i = 0; i < room.scraps.Count; i++)
            {
                if (Rectangle.Intersects(room.scraps[i].Rectangle))
                {
                    ...
                    for (int j = 0; j < rand.Next(3) + 2; j++)
                    {
                        room.gameObjects.Add(new Particle.Spark(rand, new Rectangle(Rectangle.Center.X, Rectangle.Center.Y, 2, 2), 0, room, new Vector2(rand.Next(6) - 3, rand.Next(6) - 3), false, false, true, new Color(200, 180, 0), 10, .2f));
                    }
        ...
        public override void TakeDamage(Room room, int sourceID, float damage)
        {
            ...
            room.overlays.Add(new Overlay("overlay_damage", 10,  hp / damage, Color.Red));
            room.AddShake(new Vector2(damage/hp * 5, damage/hp * 5), 4);
        }
    }
}
