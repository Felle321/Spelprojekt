...
namespace GymnasieArbete
{
    public class Room
    {
        ...
        public Room(int ID, Random rand, bool windBool, float windMagnitude, RoomReference roomReference, Room baseRoom)
        {
            if (windBool)
            {
            ...
            if (roomReference != null && baseRoom != null)
            {
                ...
                this.cleared = roomReference.cleared;
            }
        }
                                ...
                                if ((maxX * g) / h > w)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if(InstructionCollisionCheck(node, instPlatform, xPos, new Vector2(node.direction.X * Xm, Ym), measurements))
                                            ...
                                            instruction = new Instruction(instPlatform, Enemy.State.walking, new Vector2(node.direction.X, 0), xPos + (int)(measurements.X * .8f) * node.direction.X, false);
                                    }
                                }
                                else
                                {
        ...
        public bool InstructionCollisionCheck(Node node, Point instPlatform, int xPos, Vector2 movement, Point measurements)
        {
            ...
            Rectangle collisionRect = Game1.RectangleCreate(new Point(xPos, platforms[instPlatform.X].rectangle.Y), platforms[instPlatform.Y].rectangle.Center);
        ...
        public void SpawnEnemies(Random rand, float enemyPerTile, int level)
        {
            int spawnedEnemies = 0;
            bool done = false;

            while (spawnedEnemies < enemyPerTile * tiles.Count)
            {
                Enemy enemy;
                done = false;
                ...
                {
                    case (0):
                        enemy = new Enemy.LavaBot(rand, player.position, this, player.leftHand.Level, new Animation("LavaBot_Jump"), 1, 1);
                        break;
                    default:
                        return;
                }
                ...
                if (!done)
                {
                    while (possiblePlatforms.Count > 0)
                    {
                        ...
                        if (!done && platforms[i].usable)
                        {
                            for (int j = 0; j < platforms[i].rectangle.Width; j += 6)
                            {
                                if (!done)
                                {
                                    ...
                                    {
                                        if (i != k)
                                            if (!new Rectangle(platforms[i].rectangle.X + j, platforms[i].rectangle.Y - enemy.Rectangle.Height, enemy.Rectangle.Width, enemy.Rectangle.Height).Intersects(platforms[k].rectangle))
                                            {
                                                spawnedEnemies++;
