...
namespace GymnasieArbete
{
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        RenderTarget2D fogRenderTarget, mainRenderTarget;
        ...
        public bool UpdateCameraTarget()
        {
            ...
            {
                if (Math.Abs(player.Rectangle.Center.X - activeRooms[0].gameObjects[i].Rectangle.Center.X) + 350 < World.screenWidth / Camera.minZoom
                    && Math.Abs(player.Rectangle.Center.Y - activeRooms[0].gameObjects[i].Rectangle.Center.Y) + 350 < World.screenHeight / Camera.minZoom
        ...
        /*
        public void InitializeLevelOld()
        {
            //Clears the last lists of rooms and tiles
            rooms = new List<RoomReference>();
            tiles = new int[World.levelSize, World.levelSize];
            ...
            while (!done)
            {
                ...
                for (int i = 0; i < open.Count; i++)
                {
                    ...
                    if(open[i].edgeLeft || open[i].edgeRight)
                    {
                        ...
                        //This is where it chooses what room to spawn next
                        ...
                        //Randomizes the order of the rooms to get a variations
                        for (int j = 0; j < roomOrder.Length; j++)
                        {
                        ...
                        for (int j = 0; j < baseRooms.Count; j++)
                        {
                            ...
                            for (int k = 0; k < baseRooms[roomOrder[j]].tiles.Count; k++)
                            {
                                int platform0Door, platform1Door;
                                platform0Door = -1;
                                platform1Door = -1;

                                if (TileCheckDoorCompatability(open[i], baseRooms[roomOrder[j]].tiles[tileOrder[k]], out offsetX, out platform0Door, out platform1Door))
                                {
                                    RoomReference newRoom = new RoomReference(new Point(open[i].position.X + offsetX - baseRooms[roomOrder[j]].tiles[tileOrder[k]].ID.X, open[i].position.Y - baseRooms[roomOrder[j]].tiles[tileOrder[k]].ID.Y), roomOrder[j]);
                                    if (TilesAddRoom(newRoom))
                                    {
                                        //Add the new tiles to the open list
                                        ...
                                        for (int l = 0; l < baseRooms[roomOrder[j]].tiles.Count; l++)
                                        {
                                            ...
                                            open[open.Count - 1].roomID = rooms.Count - 1;
                                        }


                                        rooms[open[i].roomID].doors.Add(new Door(baseRooms[rooms[open[i].roomID].reference].doors[platform0Door].rectangle, rooms.Count - 1, rooms[rooms.Count - 1].doors.Count, offsetX));
                                        rooms[rooms.Count - 1].doors.Add(new Door(baseRooms[rooms[rooms.Count - 1].reference].doors[platform1Door].rectangle, open[i].roomID, rooms[open[i].roomID].doors.Count - 1, -offsetX));


                                        if (offsetX == 1)
                                            open[i].edgeRight = false;
                                        else
                                            open[i].edgeLeft = false;
            ...
            player = new Player(new Rectangle(200, 500, 50, 110), 0, activeRooms[0]);
            player.room = tiles[World.levelSize / 2, World.levelSize / 2];
