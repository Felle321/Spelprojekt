...
namespace GymnasieArbete
{
    public abstract class Enemy : GameObject
    {
                        ...
                        else if (platformID == path[0].platform.X)
                        {
                            ...
                            else if (position.X == path[0].xPos || (path[0].variableXPos && (Rectangle.Right < room.platforms[path[0].platform.Y].rectangle.Right && Rectangle.X > room.platforms[path[0].platform.Y].rectangle.X)))
                            {
                                ...
                                if (state == State.jump)
                                {
                                    ...
                                    else if (jumpPause && jumpTimer >= 30)
                                    {
        ...
        public virtual bool PlayerInSight(Room room)
        {
            ...
            {
                if (room.platforms[i].rectangle.Intersects(Game1.RectangleCreate(room.player.Rectangle.Center, EyePos.ToPoint())))
        ...
        public class GroundTroop : Enemy
        {
            ...
            public bool PlayerInSight1(Room room)
            {
                ...
                for (int i = 0; i < room.platforms.Count; i++)
                {
                    if (room.platforms[i].rectangle.Intersects(Game1.RectangleCreate(room.player.Rectangle.Center, Origin.ToPoint())))
            ...
            public override void Update(Room room, Random rand)
            {
                if (room.ID == room.player.room)
                {
                    ...
                    if (!jumpPause)
                    {
                        ...
                        if (!stunned)
                        {
                            if (!canAttack && state != State.charging && state != State.shooting)
                            {
                                if (room.player.position.X > position.X)
                                {
                                    direction = 1;
                            ...
                            DamageZone_Hitbox.Position = position;
                            if (PlayerInSight1(room))
                            {
                                if (canAttack && !charge && rand.Next(0, 1500) <= 10)
                                {
                                    charge = true;
                                    shootingTimer = 0;
                                    UsePath = false;
                                }
                                else if (canAttack && !charge && shootingTimer >= 60)
                                {
                                    shootingTimer = 0;
