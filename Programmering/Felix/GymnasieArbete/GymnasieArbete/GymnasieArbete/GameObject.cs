...
namespace GymnasieArbete
{
    public abstract class GameObject
    {
        ...
        public virtual void Draw(Point offset, Random rand, SpriteBatch spriteBatch, GraphicsDevice graphics, Camera camera, Color color)
        {
            if (sprite != "")
                SpriteHandler.Draw(sprite, rand, spriteBatch, camera, new Vector2(position.X + offset.X, position.Y + offset.Y), new Vector2(1,1), angle, Origin - position, color, 1f, SpriteEffects.None, 0.5f);
        }
        ...
        {
            if (spriteLight != null)
                SpriteHandler.Draw(spriteLight, rand, spriteBatch, camera, new Vector2(position.X + offset.X, position.Y + offset.Y), SpriteEffects.None, 0.5f);
        }
        ...
        {
            public int value, duration, timer;
            public float floatOffset = 0f;
            public float floatTarget = 8;
            public float floatSpeed = 2.1f;
            ...
            /// <param name="duration">0 = infinite</param>
            /// <param name="rand"></param>
            public Scrap(Rectangle rectangle, int level, Room newRoom, string spriteKey, Vector2 movement, int value, int duration, Random rand) : base(rectangle, level, newRoom, spriteKey)
            {
                sparkDelay = 60 + rand.Next(80);
                this.movement = movement;
                this.value = value;
                this.duration = duration;
                floatOffset = (float)rand.NextDouble() * floatTarget;
                timer = 1;
            }
            ...
            public override void Update(Room room, Random rand)
            {
                ...
                else
                {
                    ...
                    if(timer % sparkDelay == 0)
                    {
                        room.gameObjects.Add(new Particle.Spark(rand, new Rectangle(Rectangle.Center.X, Rectangle.Center.Y + (int)floatOffset, 2, 2), 0, room, new Vector2((float)rand.NextDouble() * 12 - 6, (float)rand.NextDouble() * 8 - 12), Vector2.Zero, .9f, true, true, true, new Color(220, 180, 0), 15, true, false, .7f));
                    }

                    floatOffset = floatTarget * (float)Math.Sin(MathHelper.ToRadians(timer * floatSpeed)) - floatTarget;

                    textureOffset.Y += floatOffset;

                    if (floatOffset == floatTarget)
                    {
                        floatTarget = -floatTarget;
                    }

                    if (Game1.GetDistance(Rectangle.Center, room.player.Rectangle.Center) < 200)
                    {
                        useGravity = false;
                        movement += Game1.GetVector2(Rectangle.Center, room.player.Rectangle.Center) * .5f / Game1.GetDistance(Rectangle.Center, room.player.Rectangle.Center);
                    }
                    else
                        useGravity = true;
            ...
            {
                SpriteHandler.Draw(sprite, rand, spriteBatch, camera, new Vector2(position.X + Rectangle.Width / 2 + offset.X, position.Y + offset.Y + floatOffset), new Vector2(1, 1), angle, Origin - position, color, 1f, SpriteEffects.None, 0.5f);
            }
        }
